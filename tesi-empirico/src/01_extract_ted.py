...
def choose_query_template(client: TedClient, iso3: str) -> tuple[str, bool]:
    ...
    for i, tpl in enumerate(API_CFG["query_templates"]):
        ...
        print(f"  template {i} OK, totalNoticeCount={total}")
        if total == 0:
            # syntactically accepted but suspicious; try to validate anyway
            print(f"  template {i}: 0 results on probe window — trying next")
            continue
...
def hierarchy_expanded(client: TedClient, query: str) -> bool:
    ...
    for n in resp.get("notices", []):
        for c in extract_cpv_list(n.get("classification-cpv")):
            if c.startswith("72") and c != "72000000":
                return True
    return False
...
def probe_fields(client: TedClient, template: str, iso3: str) -> list[str]:
    ...
    for _ in range(len(API_CFG["fields_superset"]) + 2):
        ...
        if code == 400 and isinstance(resp, str):
            # try to identify the offending field name in the error text
            bad = [f for f in fields if f in resp]
            if bad:
                for b in bad:
                    fields.remove(b)
                    dropped.append(b)
                continue
            # unidentifiable: bisect
            if len(fields) > 1:
                half = len(fields) // 2
                code2, _ = client.post({
...
def main():
    ...
    first_iso3 = next(iter(countries.values()))["iso3"]
    print(f"== Startup probing (via {first_iso3}, 2024-03) ==")
    template, server_filtered = choose_query_template(client, first_iso3)
    print(f"chosen template: {template}")
    print(f"server-side CPV filtering: {server_filtered}")
    fields = probe_fields(client, template, first_iso3)
